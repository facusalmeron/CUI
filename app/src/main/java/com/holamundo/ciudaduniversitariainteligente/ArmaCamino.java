...
public class ArmaCamino{
    ...
    //Funcion para generar un vector con las oficinas por Edifico
    public Vector<Punto> verOficinasPorEdificio(String Edificio){
        Vector<Punto> oficinas = new Vector<>();
        for(int i=0;i<Nodos.size();i++){
            if(Nodos.elementAt(i).getEdificio().equals(Edificio) && Nodos.elementAt(i).getNombre().contains("Oficina")){
                oficinas.add(Nodos.elementAt(i));
            }
        }
        return oficinas;
    }

    //Funcion para generar un vector con los laboratorios por Edifico
    public Vector<Punto> verLaboratoriosPorEdificio(String Edificio){
        Vector<Punto> laboratorios = new Vector<>();
        for(int i=0;i<Nodos.size();i++){
            if(Nodos.elementAt(i).getEdificio().equals(Edificio) && Nodos.elementAt(i).getNombre().contains("Lab.")){
                laboratorios.add(Nodos.elementAt(i));
            }
        }
        return laboratorios;
    }
